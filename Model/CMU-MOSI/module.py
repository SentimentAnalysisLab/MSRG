...
class RGNCell(nn.Module):
    ...
    def forward(self, s_t1, x_t):
        ...
        #学习门
        i_t = torch.sigmoid(self.dense3(x_t))
        g_t = torch.tanh(self.dense4(x_t))
        read = torch.multiply(i_t, g_t)

        y_t = F.relu(forget + read)

        #残差连接
        s_t = F.relu(F.relu(self.dense5(x_t)) + y_t)
        return s_t
...
class AMLP(nn.Module):
    ...
    def forward(self, feat1, feat2, feat3):
        ...
        # 计算自适应因子
        factor1 = norm1 / (norm1 + norm2 + norm3)
        factor2 = norm2 / (norm1 + norm2 + norm3)
        factor3 = norm3 / (norm1 + norm2 + norm3)

        exp_out = factor1 * feat1 + factor2 * feat2 + feat3 * factor3
