...
from BBFN.modules.position_embedding import SinusoidalPositionalEmbedding
from BBFN.modules.multihead_attention import MultiheadAttention
from BBFN.modules.encoders import DIVEncoder
import math
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, ctr_vec=None, lengths=None, mode='l2o'):
        ...
        if x_k is None and x_v is None:
            x, _ = self.self_attn(query=x, key=x, value=x, add_mask=add_mask, mul_mask=mul_mask,attn_mask=self.attn_mask)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True) 
            x, _ = self.self_attn(query=x, key=x_k, value=x_v, add_mask=add_mask, mul_mask=mul_mask,attn_mask=self.attn_mask)
