...
class MyRGN(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        for word in tf.unstack(x[:, -1:, :], axis=1):  # word: [128, 128] 250个
            state1 = out1 = self.rmn_cell1(state1, word)

        # 末层最后一个输出作为分类网络的输入: [b, 64] => [b, 1]
        out = self.outlayer(tf.concat([out0, out1], axis=1))
        # 通过激活函数，p(y is pos|x)
        prob = tf.sigmoid(out)
        return prob
