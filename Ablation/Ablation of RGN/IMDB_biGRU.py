...
class MyGRU(tf.keras.Model):    # 两层LSTM 网络， 使用LSTMCell实现
    # Cell 方式构建多层网络
    def __init__(self, units):
        ...
        self.gru_cell0 = tf.keras.layers.GRUCell(64, activation=None)
        self.gru_cell1 = tf.keras.layers.GRUCell(64, activation=None)
    ...
    def call(self, inputs, training=None):
        ...
        for word in tf.unstack(x, axis=1): # word: [b, 100]
            out0, state0 = self.gru_cell0(word, state0, training=training)
        state1 = self.state1
        for word in tf.unstack(x[:, -1:, :], axis=1):  # word: [b, 100]
            out1, state1 = self.gru_cell1(word, state1, training=training)
        output = tf.sigmoid(self.outlayer(tf.concat([out0, out1], axis=1)))
        # 通过激活函数，p(y is pos|x)
